...
export default function CommunityScreen() {
  ...
  // Child photo comes from the globally selected child in the session store
  ...
  const openChannel = async (ch: Channel) => {
    ...
    // Subscribe to realtime inserts in this channel
    const sub = supabase
      .channel(`chat-${ch.id}`)
      .on("postgres_changes", {
    ...
    return (
      <View style={{ marginBottom: 8 }}>
        ...
        >
          ...
          {!mine ? (
            <View style={s.avt}>
              <Text style={s.avtEmoji}>{msg.avatar_emoji || "😊"}</Text>
            </View>
          ) : childPhoto ? (
        ...
        {/* Emoji picker — appears below message on long press */}
        {showPicker && (
            ...
            renderItem={({ item: ch }) => (
              <TouchableOpacity style={s.chCard} onPress={() => openChannel(ch)} activeOpacity={0.8}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {!!ch.description && (
                    <Text style={s.chDesc} numberOfLines={1}>{ch.description}</Text>
                  )}
                </View>
                {ch.message_count > 0 && (
      ...
      <View style={[s.hdr, { paddingTop: insets.top + 14, flexDirection: "row", alignItems: "center", gap: 12 }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {!!activeChannel.description && (
            <Text style={s.hdrSub} numberOfLines={1}>{activeChannel.description}</Text>
          )}
        </View>
...
const s = StyleSheet.create({
  ...
  chIconBox: { width: 46, height: 46, borderRadius: 14, backgroundColor: COLORS.eduLight, alignItems: "center", justifyContent: "center" },
  ...
  avt:         { width: 34, height: 34, borderRadius: 17, backgroundColor: COLORS.eduLight, alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  // Emoji picker
  emojiPicker:     { flexDirection: "row", flexWrap: "wrap", gap: 4, backgroundColor: "#fff", borderRadius: 16, padding: 8, borderWidth: 1, borderColor: COLORS.border, marginTop: 4, marginLeft: 42, alignSelf: "flex-start", shadowColor: "#000", shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 5 },
